...
const { publishFilmMessageMqtt, MqttStatusMessageEnum } = require('../utils/Mqtt');
...
exports.createFilm = async function (film, ownerId) {
  try {
    ...
    if (!film.private) {
      const message = new MqttFilmMessage(MqttStatusMessageEnum.INACTIVE, null, null);
      publishFilmMessageMqtt(film.id, message);
    }
