...
import { mapObjToFilm, mapObjToFilmUpdate, mapObjToReview, mapObjToUser } from './utils/Factory';
...
const getFilmReviews = async (film, pageNumber) => {
  ...
  return getJson(fetch(path, { credentials: 'include' })).then(json => {
    ...
    sessionStorage.setItem('totalItems', json.totalItems);
    if (json.totalPages != 0)
      return json.reviews.map((review) => mapObjToReview(review));
    else
      return [];
  }).catch(err => {
...
async function getUsers(filmManager) {
  ...
  if (response.ok) {
    return responseJson.users.map((u) => mapObjToUser(u));
  } else {
