		...
		return new StreamObserver<ConversionRequest>() {
			...
			public void onCompleted() {
				...
				if (!typeOrigin.toString().equalsIgnoreCase("png") && !typeOrigin.toString().equalsIgnoreCase("jpg")
						&& !typeOrigin.toString().equalsIgnoreCase("gif")
						&& !typeTarget.toString().equalsIgnoreCase("png")
						&& !typeTarget.toString().equalsIgnoreCase("jpg")
				...
				if (success.get()) {
					...
					responseObserver.onNext(
							ConversionReply.newBuilder().setMeta(MetadataReply.newBuilder().setSuccess(true)).build());
					...
					try {
						while ((length = bisImageToSend.read(buffer, 0, bufferSize)) != -1) {
							responseObserver.onNext(ConversionReply.newBuilder()
									.setFile(ByteString.copyFrom(buffer, 0, length)).build());
						}
					} catch (IOException e) {
						e.printStackTrace();
				...
				// Case 2: error
					...
					responseObserver.onNext(ConversionReply.newBuilder()
							.setMeta(MetadataReply.newBuilder().setSuccess(false).setError(errorMessage.toString()))
							.build());
				}
