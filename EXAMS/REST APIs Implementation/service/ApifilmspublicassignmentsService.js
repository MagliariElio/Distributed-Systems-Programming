...
exports.assignReviewBalanced = async function (loggedUserId) {
  try {
    ...
    let filmsWithNoInvitation = await dbUtils.dbAllAsync(sqlFilmsNoInvitation, [loggedUserId, loggedUserId]);
    ...
    `;
    let usersWithPendingInvitations = await dbUtils.dbAllAsync(sqlUserWithPendingInvitation, []);

    // Find the maximum number of pending invitations for any user
    let maxInvitationCount = usersWithPendingInvitations.reduce((max, reviewer) => {
    ...
    const sqlGroupReviewerPerFilm = `
      ...
      GROUP BY filmId
    `;
    let usersWithFilmIdsReviewed = await dbUtils.dbAllAsync(sqlGroupReviewerPerFilm, filmsWithNoInvitation);
